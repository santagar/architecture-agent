...
const WORKFLOW_FILES = {
  ...
  '06': '06-platform-security-assessment.md'
};
...
const WORKFLOW_ALIASES = {
  ...
  '05': '05',
  '06-platform-security-assessment': '06',
  'platform-security-assessment': '06',
  'platform-security': '06',
  'security-assessment': '06',
  '06': '06'
};
...
function runLaunch(args) {
  ...
  if (launchType === 'workflow') {
    ...
    if (workflowId === '01') {
      ...
      if (workflowId === '06') {
        lines.push(`Source mode: "${parsed.flags['source-mode'] || 'public-surface'}".`);
        lines.push(`Platform scope: "${parsed.flags['platform-scope'] || '<platform-or-product-name>'}".`);
        lines.push(`Perimeter scope: "${parsed.flags['perimeter-scope'] || '<public-domains-endpoints-entry-points>'}".`);
        lines.push('Execution model: Battery A (public perimeter exposure), then Battery B (repository/component deep dives), then Battery C (platform security architecture consolidation) when needed.');
        lines.push('Source mode behavior: `public-surface` works from public URLs/domains/docs without requiring repository code; `repository` focuses on internal repositories/components; `hybrid` combines both.');
        lines.push('Expected outputs: perimeter report, findings by severity, remediation plan, and prioritized deep-dive queue.');
      }
    }
...
function printHelp() {
  const help = [
    ...
    '         [--change-request "..."] [--system-scope "..."] [--related-repo <repo>] [--source-mode <mode>] [--platform-scope "..."] [--perimeter-scope "..."] [--language <lang>]',
    ...
    '  architecture-agent launch workflow 06 --source-mode public-surface --platform-scope payments --perimeter-scope "https://api.example.com, https://app.example.com"'
  ].join('\n');
...
function printLaunchHelp() {
  const help = [
    ...
    '',
    'Prompt selector:',
    '  any prompt file name without .md (for example: repository-analysis, perimeter-security-review)',
    '',
    'Most common options:',
    ...
    '  --source-mode public-surface|repository|hybrid',
    '  --platform-scope "<name>" --perimeter-scope "<targets>"',
    '  --copy | --no-copy',
    '',
    'Examples:',
